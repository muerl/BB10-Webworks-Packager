...
function processSplashScreenData(data, widgetConfig) {
    if (data["rim:splashscreen"]) {
        widgetConfig.splashscreen = data["rim:splashscreen"];
    }
}
...
function validateConfig(widgetConfig) {
    ...
    //Check files specified in splash screen tag
    if (widgetConfig.splashscreen) {
        widgetConfig.splashscreen = sanitize(widgetConfig.splashscreen).trim();

        widgetConfig.splashscreen.split(":").forEach(function (file) {
            if (file !== "") {
                check(file, localize.translate("EXCEPTION_INVALID_SPLASH_SCREEN_IMG")).is(/\.(png|jpg)$/i);
            }
        });
    }
}
